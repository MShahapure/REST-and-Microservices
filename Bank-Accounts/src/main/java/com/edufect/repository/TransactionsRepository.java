...
public interface TransactionsRepository extends JpaRepository<Transactions, Long> {
	...
	Page<Transactions> findByAccounts3(Accounts accounts3, Pageable pageable);

	@Query("SELECT t FROM Transactions t WHERE t.accounts3 =:accounts3 AND t.amount > 0")
	Page<Transactions> findByDeposit(Accounts accounts3, Pageable pageable);

	@Query("SELECT t FROM Transactions t WHERE t.accounts3 =:accounts3 AND t.amount < 0")
	Page<Transactions> findByWithdraw(Accounts accounts3, Pageable pageable);
...
//	
	@Query("SELECT SUM(t.amount) FROM Transactions t WHERE t.accounts3 =:accounts3")
	Optional<Long> getCurrentBalance(@Param("accounts3") Accounts accounts);
